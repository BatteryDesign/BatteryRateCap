...
def test_potential_rate_paper_set():
    '''
    This functions tests that the input excel file has the required format, with
    sheet names containing with "C_".
    '''
    # Test paper number type
    try:
        df_cap_rate = potential_rate_paper_set(IN_PATH, ['2C_discharge'],
                                               OUT_PATH, 32, 2)
    except Exception as err:
        assert isinstance(err, AssertionError), "Function outputs the wrong error type \
                                            when input paper number is not a string." 
    ...
    try:
        ...
        assert isinstance(err, AssertionError), "Function outputs the wrong error type \
                                            when input paper number is in wrong format."         
    # Test input sheetname type
    try:
        df_cap_rate = potential_rate_paper_set(IN_PATH, '2C_discharge',
                                               OUT_PATH, 'Paper # 32', 2)
    except Exception as err:
        assert isinstance(err, TypeError), "Function outputs the wrong error type \
                                            when input sheetnames are not in a list."        

def test_potential_rate_all():
    '''
    This function tests that the input dicharge data is stored in the requried format, with
    sheet name starting with "C_".
    '''
    try:
        df_cap_rate = potential_rate_all(WRONGH_IN_PATH, OUT_PATH)
    except Exception as err:
        assert isinstance(err, AssertionError), "Function outputs the wrong error type \
                                            when input file has wrong headers."
    try:
        df_cap_rate = potential_rate_all(WRONGS_IN_PATH, OUT_PATH)
    except Exception as err:
        assert isinstance(err, AssertionError), "Function outputs the wrong error type \
                                            when input file has wrong sheetnames."   

def test_excel_merge():
    '''
    This function tests that the input excel file name has to correct extension in xls or xlsx.
    '''
    df = pd.DataFrame()
    try:
        ...
        assert isinstance(err, AssertionError), "Function outputs the wrong error type \
                                            when output file extension is wrong."      
    try:
        excel_merge(df, '000.xls', 1)
    except Exception as err:
        assert isinstance(err, AssertionError), "Function outputs the wrong error type \
                                            when output sheet name is not string."         
...
def test_capacity_cycle():
    ...
    '''
    try:
        capacity_cycle(np.array([[1, 2, 3], [2, 3, 4], [6, 6, 6]]), 2, ['1', '2'], 'mA/cm^2', 'mAh')
    except Exception as err:
        assert isinstance(err, AssertionError), "Function outputs the wrong error type \
                                            when input array is the wrong shape."         
    try:
        capacity_cycle(np.array([[1, 2], [2, 3], [6, 6]]), 2, ['1'], 'mA/cm^2', 'mAh')
    except Exception as err:
        assert isinstance(err, AssertionError), "Function outputs the wrong error type \
                                            when input array is the wrong shape."    
