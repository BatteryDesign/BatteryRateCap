...
def test_load_data():
    ...
    if str_type != "<class 'pandas.core.frame.DataFrame'>":
        raise ValueError('The object returned is not a DataFrame.')
...
def test_get_archs():
    """
    Tests if the two objected returned from geo.get_archs() are
    lists of the same length.
    """
    archs, arch_strings = geo.get_archs(df)
    if type(archs) != list:
        raise ValueError('The first object returned is not a list.')
    if type(arch_strings) != list:
        raise ValueError('The second object returned is not a list.')
    if len(archs) != len(arch_strings):
        raise ValueError('The two objects returned have different length.')
...
def test_get_params():
    """
    Tests if the two objected returned from geo.get_params() are
    lists of the same length.
    Tests the function for arch_type = 'Overall Architecture',
    'Cathode Architecture', and 'Anode Architecture'.
    """
