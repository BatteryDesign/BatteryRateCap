...
def load_data(file, sheet):
    """
    Takes as input an Excel file name and sheet name and loads datasets of
    geometry and architecture parameters into dataframes and merges them.
    ...
    geo_df.drop(columns=['DOI', 'Cathode 3D \nCharateristic Note',
                         'Anode 3D \nCharateristic Note',
                         'SA/V [1/µm]', 'SA gain'], inplace=True)
    cols = list(geo_df.columns)
    new_cols = ['Paper', 'Set', 'Full Cell Thickness [µm]',
                'Cathode Thickness [µm]', 'Anode Thickness [µm]',
                'Separator Thickness [µm]',
                'Cathode Length [µm]', 'Anode Length [µm]']
    ...
    arch_df = pd.read_excel('3D_Battery_Parameter_Log.xls',
                            sheet_name='GeneralInformation', nrows=100)
    ...
    concentric_3d_filter = arch_df['Overall Architecture'] == '3D Concentric'
    arch_df.drop(arch_df[concentric_3d_filter].index,
                 inplace=True)
...
def get_archs(merged_df):
    """
    Takes as input a dataframe and returns a list of lists of architectures and
    a list of strings for each architecture type.
    ...
    """
    # Creates a list of architectures for each type:
    # overall, cathode, and anode
    overall_arch = \
        list(np.unique(list(merged_df['Overall Architecture'])))
    cathode_arch = \
        list(np.unique(list(merged_df['Cathode Architecture'].values)))
    ...
    arch_strings = ['Overall Architecture', 'Cathode Architecture',
                    'Anode Architecture']
...
def get_params(merged_df, arch_type):
    """
    Takes as input a dataframe and an architecture type and returns a list of
    values of each parameter and the errors for each architecture type.

    Parameters:
    merged_df: dataframe
    arch_type: architecture type
            - Options: 'Overall Architecture', 'Cathode Architecture',
            'Anode Architecture'
    ...
    for arch in archs[i]:
        filter = merged_df[arch_strings[i]] == arch
        filtered_df = merged_df[filter]
        # Full cell thickness
        avg_full_cell_thickness = \
            filtered_df['Full Cell Thickness [µm]'].mean()
        avg_full_cell_thicknesses.append(avg_full_cell_thickness)

        min_full_cell_thickness = filtered_df['Full Cell Thickness [µm]'].min()
        max_full_cell_thickness = filtered_df['Full Cell Thickness [µm]'].max()
        full_cell_thickness_errors.append([avg_full_cell_thickness -
                                           min_full_cell_thickness,
                                           max_full_cell_thickness -
                                           avg_full_cell_thickness])

        # Cathode thickness
        avg_cathode_thickness = filtered_df['Cathode Thickness [µm]'].mean()
        avg_cathode_thicknesses.append(avg_cathode_thickness)

        min_cathode_thickness = filtered_df['Cathode Thickness [µm]'].min()
        max_cathode_thickness = filtered_df['Cathode Thickness [µm]'].max()
        cathode_thickness_errors.append([avg_cathode_thickness -
                                         min_cathode_thickness,
                                         max_cathode_thickness -
                                         avg_cathode_thickness])

        # Anode thickness
        avg_anode_thickness = filtered_df['Anode Thickness [µm]'].mean()
        avg_anode_thicknesses.append(avg_anode_thickness)

        min_anode_thickness = filtered_df['Anode Thickness [µm]'].min()
        max_anode_thickness = filtered_df['Anode Thickness [µm]'].max()
        anode_thickness_errors.append([avg_anode_thickness -
                                       min_anode_thickness,
                                       max_anode_thickness -
                                       avg_anode_thickness])

        # Separator thickness
        avg_sep_thickness = filtered_df['Separator Thickness [µm]'].mean()
        avg_sep_thicknesses.append(avg_sep_thickness)

        min_sep_thickness = filtered_df['Separator Thickness [µm]'].min()
        max_sep_thickness = filtered_df['Separator Thickness [µm]'].max()
        sep_thickness_errors.append([avg_sep_thickness - min_sep_thickness,
                                     max_sep_thickness - avg_sep_thickness])

        # Cathode length
        avg_cathode_length = filtered_df['Cathode Length [µm]'].mean()
        avg_cathode_lengths.append(avg_cathode_length)

        min_cathode_length = filtered_df['Cathode Length [µm]'].min()
        max_cathode_length = filtered_df['Cathode Length [µm]'].max()
        cathode_length_errors.append([avg_cathode_length - min_cathode_length,
                                      max_cathode_length - avg_cathode_length])

        # Anode length
        avg_anode_length = filtered_df['Anode Length [µm]'].mean()
        avg_anode_lengths.append(avg_anode_length)

        min_anode_length = filtered_df['Anode Length [µm]'].min()
        max_anode_length = filtered_df['Anode Length [µm]'].max()
    ...
    params = [avg_full_cell_thicknesses, avg_cathode_thicknesses,
              avg_anode_thicknesses, avg_sep_thicknesses,
              avg_cathode_lengths, avg_anode_lengths]

    errors = [full_cell_thickness_errors, cathode_thickness_errors,
              anode_thickness_errors, sep_thickness_errors,
...
def plot_geo_params(merged_df, arch_type, param, save_plot):
    """
    Takes as input an architecture type and parameter and produces plots
    of the input parameter vs. all other parameters.
    ...
    param: parameter to be plotted on the y-axis vs. all other parameters
            - Options: 'Average Full Cell Thickness',
            'Average Cathode Thickness', 'Average Anode Thickness',
            'Average Separator Thickness',
            'Average Cathode Length', 'Average Anode Length'
    ...
    param_strings = ['Average Full Cell Thickness',
                     'Average Cathode Thickness', 'Average Anode Thickness',
                     'Average Separator Thickness',
                     'Average Cathode Length', 'Average Anode Length']
    archs, arch_strings = get_archs(merged_df)
    ...
    for k in range(len(params)):
        ...
        axes[k].set_title(f'{arch_strings[i]}:' +
                          f' {y_string} vs. {param_strings[k]}')
...
def interactive_plot_geo_params(df, save_fig=False):
    ...
    plot = interact(plot_geo_params,
                    merged_df=fixed(df),
                    arch_type=Dropdown(options=arch_strings,
                                       value='Overall Architecture',
                                       description='Architecture type'),
                    param=Dropdown(options=param_strings,
                                   value='Average Full Cell Thickness',
