...
from ipywidgets import interact, interactive, Layout, Dropdown, IntSlider, Checkbox
...
def load_data():
    """
    Loads datasets of geometry and architecture parameters into dataframes and merges them. Returns the merged dataframe.
    
    Returns:
    merged_df: a dataframe containing geometry and architecture parameters
    """
    # Creates a dataframe of geometry parameters
    geo_df = pd.read_excel(io='3D_Battery_Parameter_Log.xls', sheet_name='GeometryParameters')
...
def get_archs():
    """
    Takes as input a dataframe and returns a list of lists of architectures and a list of strings 
    ...
    overall_arch = list(np.unique(list(merged_df['Overall Architecture'].values)))
...
def get_params(arch_type):
    """
    Takes as input an architecture type and returns a list of values of each parameter and the errors for 
...
def plot_geo_params(arch_type, param, save_plot):
    """
    ...
    """
    merged_df = load_data()
    params, errors = get_params(arch_type)
    param_strings = ['Average Full Cell Thickness', 'Average Cathode Thickness', 'Average Anode Thickness', 
                     'Average Separator Thickness', 'Average Cathode Length', 'Average Anode Length']
    archs, arch_strings = get_archs()
...
def interactive_plot_geo_params(save_fig=False):
    """
...
def save_plots():
    ...
    """
    arch_strings = get_archs(load_data())[1]
    param_strings = ['Average Full Cell Thickness', 'Average Cathode Thickness', 'Average Anode Thickness', 
                     'Average Separator Thickness', 'Average Cathode Length', 'Average Anode Length']
